class IndividualShift < ApplicationRecord
  ...
  def start_end_check
    if self.start.present? && self.finish.present?
      errors.add(:finish, "が開始時刻を上回っています。正しく記入してください。") if self.start >= self.finish
    end
  end
  ...
  #空きシフトは背景を黄色で表示
  def backgroundColor
    if self.number == 0 && self.finish != nil
      if self.mode == nil
        "yellow"
  ...
  def temp_color
    same_time = self.master.individual_shifts.where(start:self.start).where(temporary: false).count
    unless allDay 
      if same_time == 1
        "red"
      else
